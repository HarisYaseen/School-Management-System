...
ipcMain.handle('get-dashboard-stats', () => {
    try {
        ...
        const generalExpenses = roundMoney(queryOne("SELECT SUM(amount) as s FROM expenses")?.s || 0);
        const salaryExpenses = roundMoney(queryOne("SELECT SUM(net_paid) as s FROM salary_payments")?.s || 0);
        const expenses = roundMoney(generalExpenses + salaryExpenses);
