...
function buildSidebar(activePage) {
    ...
    return `
    <aside class="w-24 hover:w-72 bg-prime text-white flex flex-col h-screen shadow-2xl relative z-20 transition-all duration-300 ease-in-out group flex-shrink-0">
        ...
        <img src="x" style="display:none" onerror="(function(el){
            ...
            const nav = aside.querySelector('.sidebar-scroll');
            if(!nav) return;
            ...
            const forceScroll = () => {
                ...
                attempts++;
                if (attempts < 20) setTimeout(forceScroll, 50);
            };

            forceScroll();
            nav.onscroll = () => {
                if (nav.scrollTop > 0) localStorage.setItem('sidebar_scroll', nav.scrollTop);
            };
            el.remove();
        })(this)">
    </aside>`;
